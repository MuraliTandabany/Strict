...
namespace Strict.Compiler
{
    public class Parser
    {
        ...
        private static bool IsLevel0Operator(Token token) =>
            token != null && token.TokenType == TokenType.Operator &&
            Opslevel0.Contains(token.Value);

        private static bool IsLevel1Operator(Token token) =>
            token != null && token.TokenType == TokenType.Operator &&
            Opslevel1.Contains(token.Value);

        private static bool IsLevel2Operator(Token token) =>
            token != null && token.TokenType == TokenType.Operator &&
            Opslevel2.Contains(token.Value);

        private static bool IsLevel3Operator(Token token) =>
            token != null && token.TokenType == TokenType.Operator &&
            Opslevel3.Contains(token.Value);
        ...
        public ICommand CompileNestedCommandList(int newIndentation)
        {
            ...
            try
            {
                ...
                var commands = new List<ICommand>();
                LexicalAnalyzer.PushIndent(newIndentation);
                for (var command = CompileCommand(); command != null;
        ...
        private ICommand CompileSimpleCommand()
        {
            ...
            return expressionCommand.Expression switch
            {
                ...
                AttributeExpression => CompileAttributeExpression(expressionCommand,
                    valueExpression),
        ...
        private ICommand CompileClassCommand() =>
            throw
                // TODO : throw new NotImplementedException(nameof(CompileClassCommand));
                new NotImplementedException(nameof(CompileClassCommand));
        ...
        private IExpression CompileOrExpression()
        {
            ...
            while (TryCompile(TokenType.Name, "or"))
                expression = new BooleanExpression(expression, CompileAndExpression(),
                    BooleanOperator.Or);
            return expression;
        }
        ...
        private IExpression CompileAndExpression()
        {
            ...
            while (TryCompile(TokenType.Name, "and"))
                expression = new BooleanExpression(expression, CompileNotExpression(),
                    BooleanOperator.And);
            return expression;
        }
        ...
        {
            if (TryCompile(TokenType.Name, "not"))
                return new NotExpression(CompileNotExpression());
        ...
        private IExpression CompileSimpleTerm()
        {
            ...
            case TokenType.Real:
                return new ConstantExpression(Convert.ToDouble(token.Value,
